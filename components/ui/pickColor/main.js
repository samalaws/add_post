...
// function to add eventlistener to all elements in game-field
    ...
    const el =  css => document.querySelector(css); // function to search and select element from HTML file
    const create = html => document.createElement(html); // function to create new element in HTML file
...
// cheack what option was selected
// 16, 36 or 64 square
...
// 16, 36, or 64 square
    ...
    function createPlayFiled(){
        for ( let i = 0; i < checkBox; i++){
            ...
            if(colorArray[i].split(' ').join('') === whiteRGB ){ // if white color was created
...
//        console.log(masterSquare.style.backgroundColor);
        ...
        if(eColor == mColor){ // if the colors match each other

//          console.log("right color");

            element.style.backgroundColor = whiteRGB; // set white color to selected scaure
            for( i = 0; i < colorArray.length; i++){ // for loop to loop on all index of the color array
                if(colorArray[i] === mColor){ // if master color has found in  color array
            ...
            if(colorArray.length<1){ // if the colors in the array less than 1
                createReplayField() // create replay field to show the score, time, and replay button
                success.play(); // play success sound 
            }

        }else{ // if the colors of the  clicked square and refrence square match not each other
//            console.log("wrong"); 
